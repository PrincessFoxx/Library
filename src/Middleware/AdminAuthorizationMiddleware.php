<?php
   ...
   * 
   * Is a middleware that is used to authorize the user as an admin
   * 
  ...
  class AdminAuthorizationMiddleware implements MiddlewareInterface{
     ...
     * __construct
     * 
     * Is the constructor for the AdminAuthorizationMiddleware class
     * 
     * @param ResponseFactoryInterface $responseFactory The response factory that is used to create responses
     * @param UserManager $userManager The user manager that is used to get the user
     */
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface {
      ...
      if(!Overflow::validUser(UserTypes::ADMIN, $user)) { // If the user is not an admin
      ...
      return $handler->handle($request); // Handle the request
    }
  }